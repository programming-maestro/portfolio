...
(function(){
  const script = Array.from(document.getElementsByTagName('script')).reverse().find(s=>s.src && s.src.includes('template.js'));
  let base = '';
  // relative base heuristic: if path contains /companies/ or /projects/ go up one level
  if (window.location.pathname.includes('/companies/') || window.location.pathname.includes('/projects/')) base = '../';
  else base = '';
  ...
  function t(key){
    const strings = {
      en: {
        home: 'Home', companies:'Companies', projects:'Projects', download:'Download Resume', contact:'Contact', theme:'Theme', lang:'EN'
      },
      hi: {
        home: 'होम', companies:'कंपनियाँ', projects:'परियोजनाएँ', download:'रिज़्यूमे डाउनलोड', contact:'संपर्क', theme:'थीम', lang:'HI'
      }
    };
  ...
  <footer class="footer container">
    <div>© <strong>Chetan Maikhuri</strong> — Quality Engineering Leader. PDFs included in /assets/pdfs.</div>
  </footer>
  `;
    ...
    setTimeout(()=>{
      document.querySelectorAll('[data-nav]').forEach(a=>{
        try{
          ...
          if (window.location.pathname.includes('/companies/') && href && href.includes('companies')) a.classList.add('active');
          if (window.location.pathname.includes('/projects/') && href && href.includes('projects')) a.classList.add('active');
        } catch(e){}
      });
    },50);
